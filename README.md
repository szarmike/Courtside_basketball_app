...
- Puter AI is the primary speech-to-text provider. The app records six-second audio clips, sends them to `puter.ai.speech2txt()` in order using `gpt-4o-mini-transcribe`, and supplies basketball vocabulary context. If Puter cannot load, is not authorized, fails, or reaches a credit/quota limit, the current listening session automatically switches to the browser recognizer. The browser fallback prefers an installed on-device English model where supported; phones use short recognition sessions because Android's browser recognizer is unreliable in continuous mode. Missing transcripts are not invented or silently recorded.
