...
- Recognition prefers an installed on-device English model where supported. Enable offline speech offers a user-triggered language-pack download. Phones use short recognition sessions and quietly restart after each utterance because Android's browser recognizer is unreliable in continuous mode. Cloud service failures use bounded retries; after three failures on mobile, voice pauses with a clear retry action while the sound meter stays active. Unavailable browser/provider services are explained without claiming the user’s entire network is disconnected. Missing transcripts are not invented or silently recorded.
