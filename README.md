...
- A Whole game transcript tab keeps every recorded entry in order with its game time, original words, parsed actions, and Voice, Typed, or Touch label. Filter it to voice or typed entries, copy it, or download it as a text file.
...
- A visible Transcription selector switches at any time between Puter AI and Browser speech and remembers the choice on that device. Puter is the default: the app records six-second audio clips, sends them to `puter.ai.speech2txt()` in order using `gpt-4o-mini-transcribe`, and supplies basketball vocabulary context. If Puter cannot load, is not authorized, fails, or reaches a credit/quota limit, the current listening session automatically switches to the browser recognizer. The browser path prefers an installed on-device English model where supported; phones use short recognition sessions because Android's browser recognizer is unreliable in continuous mode. Missing transcripts are not invented or silently recorded.
