...
function markPlayers(text,d,normalize){
  ...
  text=text.replace(/(?<![\w@])#?(\d{1,3})(?!\w)/g,(m,n,offset)=>{const before=text.slice(0,offset),after=text.slice(offset+m.length);if(/\b(point|points|pointer|ball|defenders?|seconds?|minutes?)\b/.test(after.slice(0,18).trim().split(/[,.;]/)[0])&&/^\s*(?:point|points|pointer|ball|defenders?|seconds?|minutes?)\b/.test(after))return n;if(/\b(?:for|from|made|makes?|missed|misses?|hits?|hit|a|the|that|gets?|got|takes?|launches?|shoots?|number)\s*$/.test(before)&&!(/\b(?:player|number)\s*$/.test(before)&&!/(?:thats|that is)\s+number\s*$/.test(before)))return n;if(/^\s+for\s+[1-3]\b/.test(after))return n;if(/^[123]$/.test(n)&&/^\s+(?:no good|good|is good|is off|is short|is long|nope)\b/.test(after)&&!/^\s*$/.test(before))return n;if(/^[123]$/.test(n)&&/^\s+(?:no good|good)\b/.test(after)&&/^\s*$/.test(before))return n;return choose(d.roster.filter(p=>String(p.number)===n),before.slice(-24));});
...
export function parseNarration(raw,state,helpers){
  ...
  const orderedRefs=[...text.matchAll(/@p(\d+)/g)].map(m=>refs[+m[1]]);const ids=orderedRefs.map(p=>p.id);const unique=[...new Set(ids)];
  // Substitutions are one atomic transaction at one official clock timestamp.
  const sub=/\b(?:replaces?|entered|checks? (?:in|out)|coming (?:in|out)|going (?:in|out)|goes? to the bench|swap)\b|@p\d+\s+(?:(?:comes?|goes?) )?(?:in|out)\b|\b(?:put|take|get)\s+@p\d+\s+(?:in|out)\b/.test(text);
  if(sub&&refs.length){let outgoing,incoming;if(unique.length>=2){const a=orderedRefs[0],b=orderedRefs.find(p=>p.id!==a.id);if(/replaces?|checks? in for|coming in for|@p\d+\s+(?:checks? |coming |goes? |comes? )?in\b/.test(text.slice(0,text.indexOf('@p',text.indexOf('@p')+1)))){incoming=a;outgoing=b;}else{outgoing=a;incoming=b;}return {events:[{id:uid(),type:'PLAYER_OUT',playerId:outgoing.id,team:outgoing.team,...stamp},{id:uid(),type:'PLAYER_IN',playerId:incoming.id,team:incoming.team,...stamp}],clock:ti.explicit?{period:ti.period,seconds:ti.seconds}:{},label:`#${outgoing.number} out · #${incoming.number} in · ${formatTime(ti.seconds)}`,context:{}};}const p=orderedRefs[0],type=/\bout\b|bench/.test(text)?'PLAYER_OUT':'PLAYER_IN';return {events:[{id:uid(),type,playerId:p.id,team:p.team,...stamp}],clock:ti.explicit?{period:ti.period,seconds:ti.seconds}:{},label:`#${p.number} ${type==='PLAYER_IN'?'in':'out'}`,context:{}};}
  ...
  if(tokens.length){const pre=text.slice(0,tokens[0].index);for(let i=0;i<tokens.length;i++){const p=refs[+tokens[i][1]];const different=active?.id!==p.id;active=p;freshReference=true;if(different){shotResolved=false;ctx.shotHint=null;}let segment=text.slice(tokens[i].index+tokens[i][0].length,tokens[i+1]?.index??text.length);if(i===0&&/foul on|whistle on|block by|airball|bucket|turnover|thats.*on|no good/.test(pre))segment=pre+' '+segment;const next=tokens[i+1]?refs[+tokens[i+1][1]]:null;const clauses=segment.split(/[,;.!]+|\b(?:and|then|who)\b/).map(s=>s.trim()).filter(Boolean);if(!clauses.length&&next&&p.id!==next.id&&/^\s*[, ]*\s*$/.test(segment)){add('PASS',{playerId:p.id,toPlayerId:next.id,team:p.team});lastPass={from:p.id,to:next.id};recognized=true;}for(let j=0;j<clauses.length;j++)processClause(clauses[j],j===clauses.length-1?next:null);}}
