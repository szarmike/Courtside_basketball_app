...
function markPlayers(text,d,normalize){
  ...
  text=text.replace(/\bopponent\b(?!\s+@p\d+)/g,()=>{refs.push({id:null,number:'Opponent',team:'away'});return '@p'+(refs.length-1);});return {text:text.replace(/\s+/g,' ').trim(),refs};
}
...
export function parseNarration(raw,state,helpers){
  ...
  ctx.updatedAt=Date.now();let active=d.players[ctx.activeId]||(ctx.activeTeam==='away'?{id:null,number:'Opponent',team:'away'}:null),pending=ctx.pendingShot||null,lastShot=ctx.lastShot||null,lastPass=ctx.lastPass||null,lastMiss=ctx.lastMiss||null,freeShooter=ctx.freeShooter||null;
...
if(!segment.trim()&&p.team==='away'&&!p.id)recognized=true;const clauses=segment.split(/[,;.!]+|\b(?:and|then|who)\b/).map(s=>s.trim()).filter(Boolean);if(!clauses.length&&next&&p.id!==next.id&&/^\s*[, ]*\s*$/.test(segment)){add('PASS',{playerId:p.id,toPlayerId:next.id,team:p.team});lastPass={from:p.id,to:next.id};recognized=true;}for(let j=0;j<clauses.length;j++)processClause(clauses[j],j===clauses.length-1?next:null);}}
  ...
  const label=events.map(e=>e.type==='SHOT'?`${e.playerId?'#'+d.players[e.playerId]?.number:'Opponent'} · ${e.value===1?`${e.made}/${e.attempts} FT`:e.value+'PT '+(e.made?'MADE':'MISSED')}`:e.type==='PASS'?`#${d.players[e.playerId]?.number} → #${d.players[e.toPlayerId]?.number} PASS`:e.type==='STAT'?`${e.playerId?'#'+d.players[e.playerId]?.number:'Opponent'} · ${e.stat.toUpperCase()}`:e.type.replaceAll('_',' ')).join(' · ');
