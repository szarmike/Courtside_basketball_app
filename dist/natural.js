...
function markPlayers(text,d,normalize){
  const opponentName=normalize(d.settings.away).trim();
  if(opponentName&&opponentName!=='opponent')text=text.replace(new RegExp(`(?<![\\w])${reEscape(opponentName)}(?![\\w])`,'g'),'opponent');
  ...
  function choose(candidates,before){const team=/\b(opponent|their|them)\s*$/.test(before)?'away':'home';let list=candidates.filter(p=>p.team===team);if(list.length>1){const playing=list.filter(p=>d.players[p.id]?.onCourt);if(playing.length===1)list=playing;}if(list.length!==1)throw Error(list.length?'That player name is ambiguous. Use a unique alias or jersey number.':'Player not found on that team. Add the player before recording the play.');refs.push(list[0]);return ` @p${refs.length-1} `;}
  ...
  text=text.replace(/\b(opponent|our team|my team)\b(?!\s+@p\d+)/g,(_,word)=>{const team=word==='opponent'?'away':'home';refs.push({id:null,number:d.settings[team],team});return '@p'+(refs.length-1);});return {text:text.replace(/\s+/g,' ').trim(),refs};
}
...
export function parseNarration(raw,state,helpers){
  ...
  ctx.updatedAt=Date.now();let active=d.players[ctx.activeId]||(['home','away'].includes(ctx.activeTeam)?{id:null,number:d.settings[ctx.activeTeam],team:ctx.activeTeam}:null),pending=ctx.pendingShot||null,lastShot=ctx.lastShot||null,lastPass=ctx.lastPass||null,lastMiss=ctx.lastMiss||null,freeShooter=ctx.freeShooter||null;
  const events=[],choices=[],notes=[],atomicClauses=[],failedClauses=[];let freshReference=false,shotResolved=false,pause=false,recognized=false,currentClause='';
  const add=(type,extra={})=>{const e={id:uid(),type,...stamp,...extra,atomicClause:currentClause};events.push(e);return e;};
  ...
  function setOutcome(made,clause,{forceNew=false,value=null,attempts=1}={}){
    const p=requirePlayer();recognized=true;value=value||shotValue(clause)||pending?.value||((freeShooter!==null&&freeShooter===p.id)?1:null)||ctx.shotHint||(p.team==='away'&&!p.id?2:null);
  ...
  function processClause(clause,nextPlayer){
    clause=clause.trim();if(!clause)return;currentClause=clause;atomicClauses.push(clause);
    ...
    if(/\bthey\b/.test(clause)&&active?.team!=='away')active={id:null,number:d.settings.away,team:'away'};
    if(SELF_CREATE.test(clause)&&lastPass?.to===active?.id)lastPass=null;
    if(/\b(?:at the line|gets? fouled(?: shooting)?|was fouled(?: shooting)?)\b/.test(clause)){const p=requirePlayer();freeShooter=p.id;recognized=true;if(/fouled/.test(clause)){const foul=add('STAT',{stat:'pf',team:p.team==='home'?'away':'home',playerId:null,drawnBy:p.id});pause=true;if(lastShot?.playerId===p.id&&lastShot.made>0&&lastShot.value!==1)add('AND_ONE_PENDING_FREE_THROW',{playerId:p.id,team:p.team,shotId:lastShot.id});else if(/shooting/.test(clause))foul.expectedFreeThrows=shotValue(clause)===3?3:2;}clause=clause.replace(/\b(?:at the line|gets? fouled(?: shooting)?|was fouled(?: shooting)?)\b/g,'');}
    ...
    const reb=clause.match(REBOUND);if(reb){if(!freshReference&&(!refs.length||active?.team==='away')&&!/\b(they|their|his|her|own)\b/.test(clause))active={id:null,number:d.settings.home,team:'home'};const p=requirePlayer();if(pending){const shooter=d.players[pending.playerId]||(pending.team==='away'?{id:null,team:'away',number:'Opponent'}:null),savedActive=active;active=shooter;setOutcome(0,'missed',{value:pending.value});active=savedActive;}let stat=/offensive|\bo board|\bown\b|follows.*own|get.*again/.test(clause)?'oreb':/defensive|\bd board/.test(clause)?'dreb':lastMiss?(lastMiss.team===p.team?'oreb':'dreb'):null;const e=add('STAT',{stat:stat||'reb',playerId:p.id,team:p.team});if(!stat&&p.id)choices.push({eventId:e.id,kind:'rebound',label:`#${p.number}: offensive or defensive rebound?`});recognized=true;pending=null;lastPass=null;shotResolved=false;freshReference=true;ctx.shotHint=stat==='oreb'?2:null;clause=clause.replace(reb[0],'');if(/own miss|own shot|own rebound|own board/.test(reb[0]))clause=clause.replace(/\b(?:his|their)\b/g,'');}
    ...
    if(launch&&!pending&&!shotResolved){const p=requirePlayer();pending={playerId:p.id,team:p.team,value:value||null,period:ti.period,clock:ti.seconds,raw};recognized=true;}else if(pending&&value)pending.value=value;
    ...
    if(/putback|tips? it in|tip in/.test(clause))setOutcome(1,clause,{forceNew:true,value:2});
  }
  const safeProcess=(clause,nextPlayer)=>{try{processClause(clause,nextPlayer);}catch(error){failedClauses.push({clause:clause.trim(),error:error.message});notes.push(`Could not parse “${clause.trim()}”: ${error.message}`);}};
  ...
  if(tokens.length){const pre=text.slice(0,tokens[0].index);for(let i=0;i<tokens.length;i++){const p=refs[+tokens[i][1]];
    ...
    if(i===0&&/\bthey\b.*\b(?:miss|airball|brick|no good|score|make|hit|bucket)\b/.test(pre))safeProcess(pre,null);const different=active?.id!==p.id;active=p;freshReference=true;if(different){shotResolved=false;ctx.shotHint=null;}let segment=text.slice(tokens[i].index+tokens[i][0].length,tokens[i+1]?.index??text.length);if(i===0&&/foul on|whistle on|block by|airball|bucket|turnover|thats.*on|no good/.test(pre)&&!/\bthey\b/.test(pre))segment=pre+' '+segment;const next=tokens[i+1]?refs[+tokens[i+1][1]]:null;
...
if(!segment.trim()&&!p.id)recognized=true;const clauses=segment.split(/[,;.!]+|\b(?:and|then|who)\b/).map(s=>s.trim()).filter(Boolean);if(!clauses.length&&next&&p.id!==next.id&&/^\s*[, ]*\s*$/.test(segment)){add('PASS',{playerId:p.id,toPlayerId:next.id,team:p.team});lastPass={from:p.id,to:next.id};recognized=true;}for(let j=0;j<clauses.length;j++)safeProcess(clauses[j],j===clauses.length-1?next:null);}}
  else {freshReference=false;for(const clause of text.split(/[,;.!]+|\b(?:and|then)\b/))safeProcess(clause,null);}
  if(!recognized&&!events.length)throw Error(failedClauses[0]?.error||'No basketball action was clear. Nothing was recorded. Say the player and what happened.');
  ctx.activeId=active?.id||null;ctx.activeTeam=active?.team||null;ctx.pendingShot=pending;ctx.lastShot=lastShot;ctx.lastPass=lastPass;ctx.lastMiss=lastMiss;ctx.freeShooter=freeShooter;
  const waiting=pending?`Waiting for ${pending.playerId?'#'+d.players[pending.playerId]?.number:'the opponent'}'s shot result. Say “good,” “missed,” or “airball.”`:null;
  const label=events.map(e=>e.type==='SHOT'?`${e.playerId?'#'+d.players[e.playerId]?.number:d.settings[e.team]} · ${e.value===1?`${e.made}/${e.attempts} FT`:e.value+'PT '+(e.made?'MADE':'MISSED')}`:e.type==='PASS'?`#${d.players[e.playerId]?.number} → #${d.players[e.toPlayerId]?.number} PASS`:e.type==='STAT'?`${e.playerId?'#'+d.players[e.playerId]?.number:d.settings[e.team]} · ${e.stat.toUpperCase()}`:e.type.replaceAll('_',' ')).join(' · ');
  return {events,clock:{...(ti.explicit?{period:ti.period,seconds:ti.seconds}:{}),...(pause?{running:false}:{})},context:ctx,choices,confirm:choices.length>0,notes,atomicClauses,failedClauses,waiting:!!pending,label:label||waiting||notes.join(' ')||'Context updated; no stat added.',waitingLabel:waiting};
}
