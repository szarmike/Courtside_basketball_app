...
export function parseNarration(raw,state,helpers){
  ...
  function processClause(clause,nextPlayer){
    ...
    if(/\bthey\b/.test(clause)&&!refs.length&&active?.team!=='away')throw Error('Which team or player do you mean? Use a name or jersey number.');
...
if(!segment.trim()&&!p.id)recognized=true;const clauses=segment.split(/[,;.!]+|\b(?:and|then|who)\b/).map(s=>s.trim()).filter(Boolean);if(!clauses.length&&next&&p.id!==next.id&&/^\s*[, ]*\s*$/.test(segment)){add('PASS',{playerId:p.id,toPlayerId:next.id,team:p.team});lastPass={from:p.id,to:next.id};recognized=true;}for(let j=0;j<clauses.length;j++)processClause(clauses[j],j===clauses.length-1?next:null);}}
