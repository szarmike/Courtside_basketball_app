...
export function buildReport(state,scope='team'){
 ...
 for(let i=1;i<=doc.getNumberOfPages();i++){doc.setPage(i);doc.setFontSize(8);doc.setTextColor(90);doc.text('Courtside | Minutes use reported and estimated event times. Score corrections may differ from player totals.',36,doc.internal.pageSize.getHeight()-20);doc.text(`${i} / ${doc.getNumberOfPages()}`,width-60,doc.internal.pageSize.getHeight()-20);}
